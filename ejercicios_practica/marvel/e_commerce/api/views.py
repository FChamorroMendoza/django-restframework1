...
# filtra y lista con precio mayor o igual a 5.00
@api_view(http_method_names=['GET'])
...
# filtra y lista con stock menor o igual a 3
@api_view(http_method_names=['GET'])
def comic_list_filtered_less_api_view(request):
    _queryset = Comic.objects.filter(stock_qty__lte=3)
    _data = list(_queryset.values()) if _queryset.exists() else []
    return Response(data=_data, status=status.HTTP_200_OK)
